...
class LanguageModel:
    ...
    def sample(self, x, y):
        ...
        for token in self.vocab:
            ...
            weights.append(math.exp(self.log_prob(x, y, token)))

        return random.choices(token_list, weights = weights, k = 1)[0]
...
class BackoffAddLambdaLanguageModel(AddLambdaLanguageModel):
    ...
    def prob(self, x: Wordtype, y: Wordtype, z: Wordtype) -> float:
        # p(z) backsoff to 1/V
        p_z = (self.event_count[(z,)] + self.lambda_ * self.vocab_size * (1/self.vocab_size)) / (self.context_count[     ()] + self.lambda_ * self.vocab_size)
        p_z_given_y = (self.event_count[(y, z)] + self.lambda_ * self.vocab_size * p_z) / (self.context_count[(y, )] + self.lambda_ * self.vocab_size)
        return (self.event_count[(x, y, z)] + self.lambda_ * self.vocab_size * p_z_given_y) / (self.context_count[(x, y)] + self.lambda_ * self.vocab_size)
...
class EmbeddingLogLinearLanguageModel(LanguageModel, nn.Module):
    ...
    def __init__(self, vocab: Vocab, lexicon_file: Path, l2: float, epochs: int) -> None:
        ...
        # storing lexicon in an integerizer mappiong + a lits of float lists
        self.lex_list = []
        words_list = []
        with open(lexicon_file) as f:
            first_line = next(f).split()
            self.total_words = first_line[0]
            self.vec_length = int(first_line[1])
            
            for i in range(int(self.total_words)):  
                curr_lex = next(f).split()
                words_list.append(curr_lex[0])
                temp_vec = []
                # create float list
                for i in range(self.vec_length):
                    temp_vec.append(float(curr_lex[i+1]))
                # insert float list into the index returned by integerizer
                self.lex_list.append(temp_vec)

        # make to integerize object
        self.int_mapping = Integerizer(words_list)

        # set to the demensionality of the vectors
        self.dim: int = self.vec_length  

        # precompute z' matrix
        self.z_prime = torch.permute(self.vocab_tensor(), (1, 0))
    ...
    def log_prob_tensor(self, x: Wordtype, y: Wordtype, z: Wordtype) -> TorchScalar:
        ...
        # initialize all lexicon vectors
        x_torch = 0
        y_torch = 0
        z_torch = 0
        ool_index = self.int_mapping.index("OOL")

        # if word is not in lexicon or not in vocabulary
        if self.int_mapping.index(x) is None or x not in self.vocab:
            x_torch = torch.tensor(self.lex_list[ool_index])
        else:
            x_torch = torch.tensor(self.lex_list[self.int_mapping.index(x)])

        if self.int_mapping.index(y) is None or y not in self.vocab:
            y_torch = torch.tensor(self.lex_list[ool_index])
        else:
            y_torch = torch.tensor(self.lex_list[self.int_mapping.index(y)])

        if self.int_mapping.index(z) is None or z not in self.vocab:
            z_torch = torch.tensor(self.lex_list[ool_index])
        else:
            z_torch = torch.tensor(self.lex_list[self.int_mapping.index(z)])
        
        expression_x = x_torch @ self.X @ z_torch
        expression_y = y_torch @ self.Y @ z_torch
        num = expression_x + expression_y
        denom = self.logits(x, y)
        return num - denom
    ...
    def logits(self, x: Wordtype, y: Wordtype) -> Float[torch.Tensor, "vocab"]:
        ...
        expression_x = x_torch @ self.X @ self.z_prime
        expression_y = y_torch @ self.Y @ self.z_prime

        # returned the sum of all elements in the tensor 
        return torch.logsumexp(expression_x + expression_y, dim = 0)
    
    def vocab_tensor(self):
        return torch.stack([self.embedding(word) for word in self.vocab])
    def embedding(self, word):
        if (self.int_mapping.index(word) is None):
            return torch.tensor(self.lex_list[self.int_mapping.index("OOL")])
        else:
            return torch.tensor(self.lex_list[self.int_mapping.index(word)])
    ...
    def train(self, file: Path):    # type: ignore
        ...
        print(f'Training from corpus {str(file).split("/")[-1]}')

        #####################
        # find the number of trigrams
        N = 0
        ...
        for i in range(self.epochs):
            ...
            for (x, y, z) in read_trigrams(file, self.vocab):
                # calculate f_i(theta)
                regularization = (torch.sum(self.X**2) + torch.sum(self.Y**2)) * (self.l2/N)
                fi = self.log_prob_tensor(x, y, z) - regularization
                (-fi).backward()
                optimizer.step()
                optimizer.zero_grad()
                F += fi.item()
            print(f"epoch {i+1}: F = {F/N}")
        print(f"Finished training on {N} tokens")

        # Done: Implement your SGD here by taking gradient steps on a sequence
...
class ImprovedLogLinearLanguageModel(EmbeddingLogLinearLanguageModel):
    ...
    def log_prob_tensor(self, x: Wordtype, y: Wordtype, z: Wordtype) -> TorchScalar:
        ...
        # always add to denom
        feature_val = (x_torch @ self.X_oov + y_torch @ self.Y_oov)
        denom = self.logits(x, y, feature_val)
     
        # add to num conditionally
        if z == OOV:
            # fire feature
            expression_x = x_torch @ self.X @ z_torch + feature_val
            expression_y = y_torch @ self.Y @ z_torch + feature_val
        num = expression_x + expression_y

        return num - denom

    
    def logits(self, x: Wordtype, y: Wordtype, feature_val) -> Float[torch.Tensor, "vocab"]:
        """Return a vector of the logs of the unnormalized probabilities, f(xyz) * θ.
        These are commonly known as "logits" or "log-odds": the values that you 
        exponentiate and renormalize in order to get a probability distribution."""
        x_torch = 0
        ...
        # returned the sum of all elements in the tensor (addexp as it's in log space)
        return torch.logaddexp(torch.logsumexp(expression_x + expression_y, dim = 0), feature_val)
